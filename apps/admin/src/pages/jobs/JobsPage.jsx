...
const statusOrder = ['pending', 'quoted', 'approved', 'production', 'painted', 'delivered', 'cancelled'];
...
export default function JobsPage() {
  ...
  async function loadClients() {
    try {
      ...
      setClients(Array.isArray(data) ? data.filter((client) => client.status !== 'archived') : []);
    } catch (err) {
      setClients([]);
    }
  }
  ...
  async function openJob(job) {
    try {
      setLoading(true);
      const detail = await jobsService.getById(job.id);
  ...
  async function handleStatusChange(job, status) {
    try {
      setSaving(true);
      await jobsService.updateStatus(job.id, status);
      await loadJobs();
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Operación"
        title="Trabajos"
        description="Seguimiento por estado, prioridad, cliente, fechas, pagos y presupuesto asociado."
      ...
      <div className="toolbar-card jobs-toolbar">
        <div className="search-input">
          <Search size={18} />
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por título, cliente o tipo de trabajo" />
        </div>
        ...
        <span>{totalActive} trabajos activos</span>
      </div>

      {error && <p className="warning-box">{error}</p>}
      {loading && <LoadingState label="Cargando trabajos..." />}

      {!loading && !jobs.length && (
        <EmptyState
          icon={Wrench}
          title="Todavía no hay trabajos"
          description="Creá el primer trabajo asociándolo a un cliente existente."
          action={<button className="crm-button primary" type="button" onClick={openCreate}><Plus size={16} /> Nuevo trabajo</button>}
        />
      )}

      {!loading && jobs.length > 0 && (
        <div className="jobs-status-stack">
          {groupedJobs.map((group) => (
            <section className={`jobs-status-section jobs-status-${group.status}`} key={group.status}>
              <header>
                <div>
                  <h2>{group.label}</h2>
                  <p>{group.jobs.length} trabajos en este estado</p>
                </div>
              ...
              {group.jobs.length ? (
                <div className="jobs-grid">
                  {group.jobs.map((job) => (
                    <JobCard key={job.id} job={job} onOpen={openJob} onStatusChange={handleStatusChange} />
                  ))}
                </div>
              ) : (
